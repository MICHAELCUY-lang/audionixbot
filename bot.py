...
import yt_dlp
# Remove requests because it's causing compatibility problems
...
async def search_music(update: Update, context: CallbackContext, query: str) -> None:
    ...
    try:
        # Konfigurasi yt-dlp tanpa proxy atau opsi requests
        ydl_opts = {
            'quiet': True,
            'no_warnings': True,
            ...
            'ignoreerrors': True,
            'no_color': True,
            'socket_timeout': 20,  # Timeout dalam detik
            'noprogress': True,
            'geo_bypass': True,
            'nocheckcertificate': True,
            # Tanpa opsi requests atau proxy
        }
        
        # Cari video tanpa menggunakan parameter proxy
        results = []
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # Coba gunakan metode pencarian manual, bukan API requests
...
async def download_and_send_music(chat_id: int, video_id: str, context: CallbackContext) -> None:
    ...
    ydl_opts = {
        ...
        'noprogress': True,    # Don't show progress to avoid spamming logs
    }
